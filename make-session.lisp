(defpackage #:aws-sdk-cl/make-session
  (:use #:cl)
  ...
  (:import-from #:aws-sdk-cl/credentials
                #:credentials
                #:make-credentials)
  (:import-from #:aws-sdk-cl/shared-config
                #:make-shared-config
  ...
  (:import-from #:aws-sdk-cl/credentials/assume-role
  ...
  (:import-from #:assoc-utils
                #:aget)
  (:export #:make-session))
(in-package #:aws-sdk-cl/make-session)
         ...
         (cond
          ((shared-config-assume-role shared-config)
           (let* ((assume-role (shared-config-assume-role shared-config))
                  (assume-role-shared-config (make-shared-config :profile
